...
<template>
  <div>
    ...
    <div class="entries-wrapper">
      ...
      <ul class="entry">
      <li v-for="(definitionsLimited, lemmaWord) in definitionsSorted" v-bind:key="lemmaWord"
          class="leftAligned">
  ...
  computed: {
    ...
    definitionsSorted () {
      ...
      for (const value in this.definitions) {
        ...
        if (!(lemmaTerm in returnDict)) {
          returnDict[lemmaTerm] = []
        }
        returnDict[lemmaTerm].push(definition)
      }
      return returnDict
    },
